...
Ceci démontre que l'impact de chaque variable prédictrice est multiplicatif. En augmentant l'élévation de 1, on augmente $μ$ par un incrément de exp( $\beta_\text{Elevation}$)
...
À partir d'ici, nous pouvons estimé les déviance résiduelle et nulle. Dans notre modèle, la paramètre inconnu est l'ordonnée à l'origine ($\alpha$) et la pente de l'élévation ($\beta$):
...
manière qu'une distribution de Poisson mais autorise la variance à être différente de la moyenne. Le mélange entre la distribution de Poisson et la distribution Gamma peut se résumer à l'aide de deux paramètres, $µ$ et $k$ qui spécifie la distribution de la facon suivante :
