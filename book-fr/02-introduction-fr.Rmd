...
```{r, echo = TRUE, eval = FALSE}
par(mfrow=c(1,3), cex =1.4) #division de la fenêtre de diagramme en une ligne et 3 colonnes pour avoir 3 diagrammes sur la même figure.
...
C'est très commun que les jeux de données biologiques ne respectent pas
...
(donc μ diminue avec le contenu d'eau), mais σ^2^ est toujours 1.51. La
**variance est donc homogène** pour toutes les valeurs de x. Ce modèle
...
2\. C'est innaproprié d'utiliser une distribution normale pour prédire
