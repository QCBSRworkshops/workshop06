...
Il est **très commun** en écologie que lesdonditions d'application du modèle
...
Pour utiliser la bonne distribution, il faut spécifier deux choses 
...
--
2. une [fonction de lien](https://en.wikipedia.org/wiki/Generalized_linear_model#Link_function) (il y a une fonction de lien par défaut pour une distribution donnée!)
...
où $g$ est appelée **fonction de lien**. Cela permet de travailler avec des distribution autre que la loi normale.
...
Pour bien interpréter les coefficients du modèle, on doitla fonction 'inverse' de la function de lien ($g^{-1}$):
...
```{r}
1 / (1 + exp(-logit.reg$coefficient[2]))
```
