...
The *logit* function is the logarithm of the odds ( $\frac{\mu}{1-\mu}$).
...
$$\underbrace{\alpha}_\text{Intercept} + \underbrace{\beta_1}_\text{slope of 'Elevation'} \times \text{Elevation}_i + \underbrace{\beta_2}_\text{slope of 'Temperature'} \times \text{Temperature}_i$$

**Step 3.** The link between the mean of $Y_i$ and the systematic part is a logarithmic function can be written as:

$$log(\mu_i) = \alpha + \beta_1 \times \text{Elevation}_i + \beta_2 \times \text{Temperature}_i$$
.pull-left[
.center[or]

$$\mu_i = e^{ \alpha + \beta_1 \times \text{Elevation}_i + \beta_2 \times \text{Temperature}_i}$$

.center[or]

$$\mu_i = e^{\alpha} \times e^{\beta_1^{\text{Elevation}_i}} \times e^{\beta_2^{\text{Temperature}_i}}$$
