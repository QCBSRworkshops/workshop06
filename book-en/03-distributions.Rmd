...
the shape of the distribution (for example $\mu$ and $\sigma^2$ for the normal
...
a single parameter, $\lambda$ (lambda), which defines both the mean and the
...
distributions with different values of $\lambda$, corresponding in this case to
...
Our variable `$pa` (presence-absence) takes yet another form. It
...
and the mean of the distribution decline as $\lambda$ decreases with increasing
water content. Why is the fitted line of predicted values curved? Why is
this called a "generalized" linear model?
