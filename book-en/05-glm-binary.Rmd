...
When we predict the probability to observe a phenomenon $Y$ with a binary variable, the predicted value has to be between 0 and 1: it's or potentiel extent of probability! 
...
We have seen that binary variables are not normally distributed (*i.e.* we observe a peak at 0 and a peak at 1, but nothing in between). In 'R', we can specified the statistical distribution of our model with the argument 'family'. For a logistic regression, we indicate 'family = binomial'.
...
When an estimated parameter is between 0 and 1 on the odds' scale, the relationship between the response variable and the predictors is **negative**. If it's greater than 1, the relationship will be **positive**. If the confidence interval include 1 onthe odds' scale, the relationship is *not signficant*.

Keep in mind that a value of 1 ont he odds' scale indicates that the probability to oberserve the event Y is equal to the probability of *not* observing the event (*i.e.* $p$ = 0.5, 0.5/(1-0.5) = 1).

Let's try with topography:
```{r, echo = TRUE, eval = TRUE}
...
Note that the odds for a predictor is calculated when all other variables are constant. Topography as an odd of 8.09. Thus the probability of observing *Galumna* sp. is 8.09 grester when the topography is `hummock` instead of `blanket`.

We can calculate the odds without the function `exp()`:

Let's start with the odds for topography from `logit.reg`:   
$µ/ (1 - µ) = 8.09$
...
A nul model is a model without any predictor. In R, we create a nul model this way :
...
<details>
  <summary>**Clic here to se the solution!**</summary>
...
Once we've validated our model, we'll likely want to visually represent our results. Here is an example with `ggplot2`. See [workshop 3](r_workshop3)for a reminder for this package.
...
ggtitle("Probability of presence of  Galumna sp. against the water content")
```