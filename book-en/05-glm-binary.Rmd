...
variable: we observe a phenomenon $Y$ or its "absence". For example,
...
of organisms. Usually, we are interested in questions such as: how do species occurrences vary in function of the environment?
...
absence (or failure, death...) as `0`. A logistic regression (or any other generalized linear model) is performed with the `glm()` function. This function is different from the basic `lm()` as it allows one to specify a statistical distribution other than the normal distribution.
...
The following example will help you understand why a basic
...
where $g(\mu)$ is the link function for the expected values. This allows
...
where $\mu$ represents expected values (*i.e.* the probability that Y = 1
because we observed the presence of a species, disease, success, or some
other event). The ratio $\mu / 1-\mu$ represents the odds that some
...
How to obtain the odds of the slope? Use the "exp()" function to put
...
First we start with our odds ratio for topography from the logit.reg model:
**µ/ (1 - µ) = 8.09**
