...
| Exponential              | Negative Inverse           | $g(\mu) = -\mu^{-1}$  | $-\mu^{-1} = \mathbf{X} \boldsymbol{\beta}$ | `Gamma(link="inverse")` |
...
When the residual deviance is higher than the residual degrees of freedom we say that the model is **overdispersed**. We can calculate an overdispersion parameter ( $\phi$ ):
