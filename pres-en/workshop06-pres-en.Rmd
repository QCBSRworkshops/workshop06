...
![badge](https://img.shields.io/static/v1?style=flat-square&label=script&message=06&color=2a50b8&logo=r)
...
1. The slope does not follow the trend in the observation
2. the $\mu$ (mean) estimates are incorrect
3. The $\sigma^2$ (variance) is considered homogeneous
4. The normal distribution is continuous but observations are discrete
]
...
3\. $\sigma^2$ is not homogeneous, yet `lm()` forces a constant $\sigma^2$

4\. Predicted values should be integers
]
...
---
# Plus, normal distributions are not the only options!
...
$$\underbrace{g(\mu_i)}_{Link~~function}  = \underbrace{\beta_0 + \beta_1X_1~+~...~+~\beta_pX_p}_{Linear~component}~+~\underbrace{\epsilon}_{Error}$$
and,
...
For binary or binomial data, the link function is called the **logit**:

$$g(p) = log\frac{p}{1-p}$$

with $\mu$ being the expected values (probability that $Y = 1$).
...
*The expected values are now **linearly** related to the linear predictor.*
...
.center[We can use GLM!]

We can switch the distribution of error terms ($ε_i$) from Normal to Poisson:
$$Y_i \sim \color{red}N(\mu = \beta_0 + \beta_1 X_i +\varepsilon)$$

$$y_i \sim \color{red}{Poisson}(\lambda = \beta_0 + \beta_1 x_i +\varepsilon)$$
where:

1. $\lambda$ varies with $X$ (e.g., water content), with the residual variance also varying with $X$. (This also means that we have relaxed the assumption of homogeneity of variances!)

2. Predicted values are now be integers instead of fractions;
