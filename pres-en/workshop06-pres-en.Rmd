...
plot(Galumna ~ WatrCont, 
     data = mites, xlab = 'Water content', ylab='Water content')
boxplot(WatrCont ~ pa, 
        data = mites, xlab='Presence/Absence', ylab = 'Water content')
plot(prop ~ WatrCont, 
     data = mites, xlab = 'Water content', ylab='Proportion')
```
...
```
???
These graphs all represents potential relationships, but the middle one is more likely to show properly in a model (the boxes do not overlap each other)
...
???
On the right, we show graphs where wither mu varies or omega varies. The top graph would wrok with a linear model -> omega is constant throughout values of Y. The bottom is most likely what we observe, with a varying omega necessiting a different distribution.
---
...
```
???
You can say that *in average*, you expect 7.56 individuals in a given situation. But you measure something *discrete*.
---
# Modeling count data
