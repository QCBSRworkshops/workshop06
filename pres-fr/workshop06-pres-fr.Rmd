...
[![badge](https://img.shields.io/static/v1?style=flat-square&label=Diapos&message=06&color=red&logo=html5)](https://qcbsrworkshops.github.io/workshop06/pres-fr/workshop06-pres-fr.html)
[![badge](https://img.shields.io/static/v1?style=flat-square&label=livre&message=06&logo=github)](https://qcbsrworkshops.github.io/workshop06/book-fr/index.html)
[![badge](https://img.shields.io/static/v1?style=flat-square&label=wiki&message=06&logo=wikipedia)](https://wiki.qcbs.ca/r_atelier6)
...
En plus des versions les plus récentes de R et RStudio, cet atelier requiert les packages R suivants:
...
Nous cherchons souvent une fonction $f$ qui explique une variable réponse ($Y$) *en fonction* d'une ($X_1$) ou de plusieurs autres variables prédictives ( $X_2$, $X_3$, $...$, $X_n$ ):

$$Y = f(X_1)$$
--
L'ensemble de variables prédictives que nous avons mesuré ne pourra jamais complètement expliquer notre variable $Y$. En conséquence, il y a toujours une **variation imprévisible** dans nos modèles, i.e. l'erreur $\epsilon$. L'erreur est  donc une partie irrévocable de notre fonction:

$$Y = f(X_1, \epsilon)$$
---
# Révision des modèles linéaires

Dans l'[atelier 4](https://qcbsrworkshops.github.io/workshop04/pres-fr/workshop04-pres-fr.html#1), nous avons appris comment utiliser ces modèles linéaires généraux comme $f(\cdot)$ pour décrire la relation entre variables:
...
Utilisons nos connaissances acquises des modèles linéaires généraux pour explorer la relation entre les variables dans le jeu de données de *mites Orbatid*.

.small[
.pull-left3[
Commençons en chargeant les données :
...
.small[
Le jeu de données de mites contient **70 échantillons de mousses et mites** et **5 variables environmentales, l'abondance de la mite *Galumna* sp., et l'abondance totale des mites**. 
...
3\. Les limites de l'espace d'échantillonage change.
...
*C'est beaucoup moins intuitif d'interpréter que l'abondance de Galumna prend la forme de $\log(1.63).$ pour chaque augmentation de $300$ unités en contenu en eau.*
]
...
---
# Modéliser avec d'autres distributions
...
$\text{pseudo-R}^2 \text{est la variance expliquée par le modèle}$
...
Maintenant, la déviance d'un modèle qui a des estimations ${\hat {\mu }}=E[Y|{\hat {\theta }}_{0}]$ peut être définie par sa **vraisemblance** ("*likelihood*"):

$$D(y,{\hat {\mu }})=2{\Big (}\log {\big (}p(y\mid {\hat {\theta }}_{s}){\big )}-\log {\big (}p(y\mid {\hat {\theta }}_{0}){\big )}{\Big )}$$
avec $\hat \theta_0$ indiquant les valeurs ajustées des paramètres dans le modèle réduit, tandis que ${\displaystyle {\hat {\theta}}_{s}}\hat \theta_s$ indique les paramètres ajustés pour le modèle saturé.
...
### Exercise 2: Faire un test d'ajustement du modèle Hosmer-Lemeshow
...
> Les tests sur les variables explicatives apparaîtront généralement plus significatifs et les intervalles de confiance des paramètres seront plus étroits
que les données ne le justifierait!
