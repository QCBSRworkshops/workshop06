...
.center[
![:scale=50%](images/modelPredic.png)
]
]
...
---
# GLM vec données binomiales: lien logit
...
Maintenant, la déviance d'un modèle qui a des estimations ${\hat{\mu}}=E[Y|{\hat {\theta}}_{0}]$ peut être définie par sa **vraisemblance** :
...
Maintenant, la déviance d'un modèle qui a des estimations ${\hat {\mu }=E[Y|{\hat {\theta}}_{0}]$ peut être définie par sa **vraisemblance** :
$$D(y,{\hat {\mu }})=2{\Gros (}\log (}big (}p(y\mid{\theta}}_{s}){\big )}-\log (}p(y\mid{\theta}_{0}){\big )}{\Big )}$$
avec $\hat \theta_0$ indiquant les valeurs ajustées des paramètres dans le modèle réduit, tandis que ${\displaystyle {\hat {\theta}}_{s}}\hat \theta_s$ indique les paramètres ajustés pour le modèle saturé.
...
Et, la **déviation nulle** est définie comme 2 fois le logarithme du rapport de vraisemblance du modèle complet par rapport au modèle nul (c'est-à-dire que les prédicteurs sont fixés à 1).
